...
#anova
summary(model_all4gene_nomethod)
summary(model_all4genealpha_nomethod)
summary(model_all4genebeta_nomethod)
anova(model_all4gene_nomethod, btt=30:31)
anova(model_all4genealpha_nomethod, btt=30:31)
anova(model_all4genebeta_nomethod, btt=30:31)
...
ggplot(global_betas)+ geom_errorbar(aes(x=coefficient,ymin=ci.lb,ymax=ci.ub))+geom_point(aes(x=coefficient,y=beta))+coord_flip() + facet_grid(.~type) + theme_bw() + ylab("model coefficient")
...
ggplot(global_betas)+ geom_hline(yintercept=0,linetype="dashed",colour="gray") + geom_errorbar(aes(x=coefficient,ymin=ci.lb,ymax=ci.ub,color=variable,alpha=cicrosseszero),width=0,size=1.5)+scale_alpha_manual(values=c(1, 0.25),guide="none")+geom_point(aes(x=coefficient,y=beta,color=variable),size=3.5)+coord_flip() + facet_grid(.~type) + theme_bw(base_size = 14) + labs(x=NULL) + ylab("meta-analysis model coefficient and 95% confidence interval") + theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank()) + scale_color_manual(values=colors)
